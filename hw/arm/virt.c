...
static void create_gic(VirtMachineState *vms, qemu_irq *pic)
{
    ...
    if (whpx_enabled()) {
        /* WHPX emulates GICv3 in the hypervisor. Skip QEMU device creation.
         * Create IRQ objects that forward to the hypervisor via WHvRequestInterrupt. */
        for (i = 0; i < NUM_IRQS; i++) {
            pic[i] = qemu_allocate_irq(whpx_arm64_inject_irq, NULL, i);
        }
        vms->gic_phandle = qemu_fdt_alloc_phandle(vms->fdt);
        qemu_fdt_setprop_cell(vms->fdt, "/", "interrupt-parent",
                              vms->gic_phandle);
        qemu_fdt_add_subnode(vms->fdt, "/intc");
        qemu_fdt_setprop_string(vms->fdt, "/intc", "compatible", "arm,gic-v3");
        qemu_fdt_setprop_cell(vms->fdt, "/intc", "#interrupt-cells", 3);
        qemu_fdt_setprop(vms->fdt, "/intc", "interrupt-controller", NULL, 0);
        qemu_fdt_setprop_cell(vms->fdt, "/intc", "#address-cells", 0x2);
        qemu_fdt_setprop_cell(vms->fdt, "/intc", "#size-cells", 0x2);
        qemu_fdt_setprop(vms->fdt, "/intc", "ranges", NULL, 0);
        qemu_fdt_setprop_cell(vms->fdt, "/intc", "#redistributor-regions", 1);
        qemu_fdt_setprop_sized_cells(vms->fdt, "/intc", "reg",
                                     2, (uint64_t)0x08000000,
                                     2, (uint64_t)0x00010000,
                                     2, (uint64_t)0x080A0000,
                                     2, (uint64_t)(0x20000 * smp_cpus));
        qemu_fdt_setprop_cell(vms->fdt, "/intc", "phandle", vms->gic_phandle);
        return;
    }
...
static void machvirt_init(MachineState *machine)
{
    ...
    /* WHPX always emulates GICv3 — override to match the hypervisor */
    if (whpx_enabled() && vms->gic_version != 3) {
        vms->gic_version = 3;
    }
