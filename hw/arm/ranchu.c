...
static void fdt_add_gic_node(const VirtBoardInfo *vbi)
{
    ...
    if (whpx_enabled()) {
        /* WHPX: hypervisor emulates GICv3, describe it in the DTB.
         * GICD at 0x08000000, GICR at 0x080A0000 (128KB per CPU). */
        fprintf(stderr, "DBG[ranchu]: Using GICv3 DTB for WHPX (smp_cpus=%d)\n", smp_cpus);
        qemu_fdt_setprop_string(vbi->fdt, "/intc", "compatible",
                                "arm,gic-v3");
        qemu_fdt_setprop_cell(vbi->fdt, "/intc", "#interrupt-cells", 3);
        qemu_fdt_setprop(vbi->fdt, "/intc", "interrupt-controller", NULL, 0);
        qemu_fdt_setprop_cell(vbi->fdt, "/intc", "#redistributor-regions", 1);
        qemu_fdt_setprop_sized_cells(vbi->fdt, "/intc", "reg",
                                     2, (uint64_t)0x08000000,
                                     2, (uint64_t)0x00010000,
                                     2, (uint64_t)0x080A0000,
                                     2, (uint64_t)(0x20000 * (uint64_t)smp_cpus));
    } else {
...
static void create_gic(const VirtBoardInfo *vbi, qemu_irq *pic)
{
    ...
    fprintf(stderr, "DBG[ranchu]: create_gic: whpx_enabled()=%d\n", whpx_enabled());
    if (whpx_enabled()) {
        /* WHPX: hypervisor emulates GICv3. Don't create a QEMU GIC device.
         * Device IRQs (pic[]) are left NULL — devices that check before
         * asserting will skip, and the hypervisor manages interrupt delivery. */
