...
#if defined(_WIN32) && defined(TARGET_AARCH64)
/* WHPX on ARM64 uses 32-bit PCI MMIO (highmem disabled), so keep
 * the area small enough to fit in the virt machine's 32-bit window. */
#define GOLDFISH_ADDRESS_SPACE_AREA_SIZE            (256ULL << 20ULL)
