                         ...
                         tcg_target_long value)
{
    ...
    int s0, s1;
    AArch64Insn opc;
    static int dbg_movi_count = 0;
    int dbg_movi_trace = 0;
    ...
    if (type == TCG_TYPE_I32 || (value & ~0xffffffffull) == 0) {
        if (dbg_movi_trace) fprintf(stderr, "DBG[movi]: → I32 path (val_hi=0x%llx)\n", (unsigned long long)(value >> 32));
    ...
    if ((value & ~0xffffull) == 0) {
        if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → MOVZ16 val=0x%llx\n", (unsigned long long)value); dbg_movi_count++; }
        tcg_out_insn(s, 3405, MOVZ, type, rd, value, 0);
        return;
    } else if ((ivalue & ~0xffffull) == 0) {
        if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → MOVN16\n"); dbg_movi_count++; }
        tcg_out_insn(s, 3405, MOVN, type, rd, ivalue, 0);
        return;
    }

    if (is_limm(svalue)) {
        if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → LIMM\n"); dbg_movi_count++; }
    ...
    if (type == TCG_TYPE_I64) {
        ...
        if (disp == sextract64(disp, 0, 21)) {
            if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → ADR\n"); dbg_movi_count++; }
            tcg_out_insn(s, 3406, ADR, rd, disp);
            return;
        }
        disp = (value >> 12) - ((intptr_t)s->code_ptr >> 12);
        if (disp == sextract64(disp, 0, 21)) {
            if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → ADRP\n"); dbg_movi_count++; }
    ...
    t2 = t1 & ~(0xffffULL << s1);
    if (dbg_movi_trace) {
        fprintf(stderr, "DBG[movi]: MOVZ/MOVK path: t0=0x%llx s0=%d t1=0x%llx s1=%d t2=0x%llx ctpop=%d\n",
                (unsigned long long)t0, s0, (unsigned long long)t1, s1, (unsigned long long)t2, ctpop64(value));
        fprintf(stderr, "DBG[movi]: 0xffffUL=%llx sizeof(UL)=%d\n",
                (unsigned long long)0xffffUL, (int)sizeof(0xffffUL));
    }
    if (t2 == 0) {
        if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → MOVZ+MOVK (2 insns)\n"); dbg_movi_count++; }
    ...
    if (dbg_movi_trace) { fprintf(stderr, "DBG[movi]: → CONST POOL\n"); dbg_movi_count++; }
