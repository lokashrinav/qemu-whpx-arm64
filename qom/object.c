...
void object_initialize(void *data, size_t size, const char *typename)
{
    ...
    if (!type) {
        fprintf(stderr, "DBG[object.c]: object_initialize FAILED for type '%s'\n", typename);
        fflush(stderr);
    }
...
Object *object_new(const char *typename)
{
    ...
    if (!ti) {
        fprintf(stderr, "DBG[object.c]: object_new FAILED for type '%s'\n", typename);
        fflush(stderr);
    }
                                   ...
                                   const char *file, int line, const char *func)
{
    ...
    for (i = 0; obj && i < OBJECT_CLASS_CAST_CACHE; i++) {
        if (atomic_read(&obj->clazz->object_cast_cache[i]) == typename) {
            goto out;
        }
    }
    ...
    if (obj && obj == inst) {
        for (i = 1; i < OBJECT_CLASS_CAST_CACHE; i++) {
            atomic_set(&obj->clazz->object_cast_cache[i - 1],
                       atomic_read(&obj->clazz->object_cast_cache[i]));
        }
        atomic_set(&obj->clazz->object_cast_cache[i - 1], typename);
    }
