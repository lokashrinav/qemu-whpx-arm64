...
class ConfigureTask(BuildTask):
    ...
    def __init__(
        ...
        cmake = shutil.which("cmake", path=str(cmake_dir))
        if cmake is None:
            cmake_candidate = cmake_dir / ("cmake.exe" if platform.system() == "Windows" else "cmake")
            if cmake_candidate.exists():
                cmake = str(cmake_candidate)
        if platform.system() == "Windows":
            local_cmake = shutil.which("cmake")
            if local_cmake:
                cmake = local_cmake
        self.cmake_cmd = [
            cmake,
