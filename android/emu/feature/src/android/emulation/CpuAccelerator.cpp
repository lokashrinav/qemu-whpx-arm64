...
std::pair<AndroidHyperVStatus, std::string> GetHyperVStatus() {
...
#else  // _WIN32
#if defined(_M_ARM64)
    // On ARM64 Windows, CPUID is not available. Detect Hyper-V by checking
    // if WinHvPlatform.dll is loadable and reports a hypervisor present.
    HMODULE hWinHvPlatform = LoadLibraryW(L"WinHvPlatform.dll");
    if (hWinHvPlatform) {
    ...
    return std::make_pair(ANDROID_HYPERV_ABSENT,
                          "Hyper-V is not available (ARM64)");
#else  // !_M_ARM64
    char vendor_id[16];
