...
public:
    ...
    void addWatchedLooper(
            base::Looper* looper,
            std::chrono::milliseconds taskTimeout = std::chrono::seconds(600));
...
private:
    static constexpr Timing defaultTiming() {
        return {.hangLoopIterationTimeout = std::chrono::seconds(120),
                .hangCheckTimeout = std::chrono::seconds(600)};
    }
