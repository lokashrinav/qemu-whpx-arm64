...
gen_wrapper_program ()
{
    ...
    # do something completely different for ar on darwin-aarch64
    ...
    case $PROG in
        ar)
            case $CURRENT_HOST in
                darwin-aarch64)
                    log " warning you are about to get rocked by ar on darwin-aarch64 "
                    CUSTOM_GEN="1"
                    cat > "$DST_FILE" << EOF
#!/bin/bash
...
EOF
    ...
    log2 "custom gen: ${CUSTOM_GEN}"

    if [ "0" = "$CUSTOM_GEN" ]; then
...
IFS= read -r -d '' EXTRA_ENV_SETUP << 'EOM' || true

  # We need a path filter, otherwise the cross compile toolchain will fail.
...
prepare_build_for_windows_msvc() {
    ...
    if [ -d "$OPT_VSDIR/win10sdk" ]; then
      MSVC_DIR=$OPT_VSDIR/win10sdk
    elif [ -d "$OPT_VSDIR/win8sdk" ]; then
      MSVC_DIR=$OPT_VSDIR/win8sdk
    else
    ...
    if [ -d "$MSVC_DIR/Windows Kits" ]; then
      if [ ! -d "$MSVC_DIR/Windows_Kits" ]; then
        log "Setting symlink for Windows 10 kit"
        ln -sf "$MSVC_DIR/Windows Kits" "$MSVC_DIR/Windows_Kits"
      fi
...
:
    ...
    log "Using version [${MSVC_VER}], found in ${MSVC_DIR}/$WIN_SDK"
...
prepare_build_for_host () {
    ...
    case $CURRENT_HOST in
        linux-*)
            ...
            ;;
        darwin-aarch64)
            # We assume that you actually are on arm, as the architecture on M1
            # can be misreported (you could be in X86 emulation from cmake)
            if [ "$BUILD_HOST"  != "darwin" ]; then
              OSX_DEPLOYMENT_TARGET=10.16
              CURRENT_HOST=darwin-aarch64-cross
              prepare_build_for_cross_darwin arm64
            else
              prepare_build_for_darwin_aarch64
            fi
            ;;
        darwin-*)
            if [ "$BUILD_HOST"  != "darwin" ]; then
              ...
              CURRENT_HOST=darwin-x86_64-cross
              prepare_build_for_cross_darwin x86_64
            else
              prepare_build_for_darwin
            fi
            ;;
