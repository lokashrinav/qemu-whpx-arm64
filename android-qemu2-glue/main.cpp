...
#else
    args.add((hw->hw_cpu_model && hw->hw_cpu_model[0]) ? hw->hw_cpu_model : kTarget.qemuCpu);
#endif
...
#elif defined(_WIN32)
        args.add("-enable-whpx");
#else
        args.add("-enable-kvm");
#endif
    } else {
        dwarning("Hardware acceleration is not available on this aarch64 host.");
    }
#else
